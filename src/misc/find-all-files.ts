...
export async function main(ns: NS): Promise<void> {
    ...
    await ServerTree.buildTree(ns)
        .traverseAndExecute(async (host) => {
            const files = ns.ls(host).filter((file) => !file.includes("template.js"));
            if (files.length > 0) allFiles.set(host, files);
        });

    allFiles.forEach((files, host) => ns.tprint(`${host} :: ${files}`));
}